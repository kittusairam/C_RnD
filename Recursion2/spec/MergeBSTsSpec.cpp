...
#pragma endregion 
		...
		void test_mergeBSTs(struct BstNode *root,struct BstNode *Nodes[],int len) {
			for (int i = 0; i < len; i++) {
				if (0&&searchNode(root, Nodes[i]) == 0) {
		...
		void MergeBST_Test02()
		{
			...
			Assert::AreEqual(8, totalNodeCount(bst1), L"Nodes Length", 1, 2);
			test_mergeBSTs(bst1, Nodes, len);
		};
		...
		void MergeBST_Test03()
		{
			...
			Assert::AreEqual(19, totalNodeCount(bst1), L"Nodes Length", 1, 2);
			test_mergeBSTs(bst1, Nodes, len);
		};
		...
		void MergeBST_Test05()
		{
			...
			Assert::AreEqual(4, len1 + len2, L"Nodes Length1", 1, 2);
			Assert::AreEqual(0, totalNodeCount(bst1), L"Nodes Length2", 1, 2);
			test_mergeBSTs(bst1, Nodes, len);
		};
