...
As he likes jumping ,He would like to know the Path which starts at a Loose
r Pillar and ends at a Leader Pillar
...
#include <stdlib.h>
#include <stdio.h>
int ll(int *b, int n, int m, int i, int j, int x, int y) {
	if (i >= 0 && i < m && j >= 0 && j < n && b[i*n + j] != 0) {
		if (b[x*n + y] >= b[i*n + j])
			return 1;
		else
			return -1;
...
void fun(int **a, int x, int y, int i, int j, int n, int m, int *ans, int *max, int count, int *b, int *res, int **v, int nil) {
	if (i >= 0 && i < m && j >= 0 && j < n && v[i][j] == 0 && b[i*n + j] > b[x*n + y]) {
		ans[count] = b[i*n + j];
		count++;
		v[i][j] = 1;
		if (a[i][j] == 2) {
		...
		int val = 0;
		if (isMove(i - 1, j, n, m))//n
			fun(a, i, j, i - 1, j, n, m, ans, max, count, b, res, v, val);
		if (isMove(i, j + 1, n, m))//e
			fun(a, i, j, i, j + 1, n, m, ans, max, count, b, res, v, val);
		if (isMove(i + 1, j, n, m))//s
			fun(a, i, j, i + 1, j, n, m, ans, max, count, b, res, v, val);
		if (isMove(i, j - 1, n, m))//w
			fun(a, i, j, i, j - 1, n, m, ans, max, count, b, res, v, val);
		v[i][j] = 0;
		count--;
	}
...
int * pillars_max_jumps(int *b, int m, int n, int *jumps_count){
	if (b == NULL)
		return NULL;
	int i, j;
	...
	for (i = 0; i < m; i++) {
		for (j = 0; j < n; j++) {
			...
			if (d != 0) {
				int c = 0;
				if (i - 1 < 0 || b[(i - 1)*n + j] <= d)
					c++;
				if (j + 1 >= n || b[i*n + j + 1] <= d)
					c++;
				if (i + 1 >= m || b[(i + 1)*n + j] <= d)
					c++;
				if (j - 1 < 0 || b[i*n + j - 1] <= d)
					c++;
				if (c == 4)
					a[i][j] = 2;
				else {
	...
	int *ans = (int*)malloc(sizeof(int)*n*m), *res = (int*)malloc(sizeof(int)*n*m);
	int max = 1, flag = 0, flag2 = 0, flag3 = 0;
	for (i = 0; i < m; i++) {
		for (j = 0; j < n; j++) {
			if (a[i][j] == -1) {
				flag = 1;
				...
				int val = 0;
				if (isMove(i - 1, j, n, m))//n
					fun(a, i, j, i - 1, j, n, m, ans, &max, 1, b, res, v, val);
				if (isMove(i, j + 1, n, m))//e
					fun(a, i, j, i, j + 1, n, m, ans, &max, 1, b, res, v, val);
				if (isMove(i + 1, j, n, m))//s
					fun(a, i, j, i + 1, j, n, m, ans, &max, 1, b, res, v, val);
				if (isMove(i, j - 1, n, m))//w
					fun(a, i, j, i, j - 1, n, m, ans, &max, 1, b, res, v, val);
