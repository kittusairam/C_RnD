...
int maxDepth(struct student *root)
{
	...
	if (root->first == NULL){
		int a = maxDepth(root->second)+1;
		int b = maxDepth(root->third) + 1;
		if (a <= b){
			return b;
		}
		else{
	...
	if (root->second == NULL){
		int a = maxDepth(root->third) + 1;
		int b = maxDepth(root->first) + 1;
		if (a <= b){
			return b;
		}
		else{
	...
	if (root->third == NULL){
		int a = maxDepth(root->first) + 1;
		int b = maxDepth(root->second) + 1;
		if (a <= b){
			return b;
		}
		else{
	...
	int a = maxDepth(root->first);
	int b = maxDepth(root->second);
	int c = maxDepth(root->third);
	if (a <= b){
		if (b <= c){
			return c + 1;
		}
...
int minDepth(struct student *root)
{
	// Corner case. Should never be hit unless the code is 
	// called on root = NULL
	if (root == NULL)
	...
	if (root->first == NULL){
		int a = minDepth(root->second) + 1;
		int b = minDepth(root->third) + 1;
		if (a <= b){
			return a;
		}
		else{
	...
	if (root->second == NULL){
		int a = minDepth(root->third) + 1;
		int b = minDepth(root->first) + 1;
		if (a <= b){
			return a;
		}
		else{
	...
	if (root->third == NULL){
		int a = minDepth(root->first) + 1;
		int b = minDepth(root->second) + 1;
		if (a <= b){
			return a;
		}
		else{
	...
	int a = minDepth(root->first);
	int b = minDepth(root->second);
	int c = minDepth(root->third);
	if (a <= b){
		if (a <= c){
			return a + 1;
		}
...
{
	*minBallDistance = minDepth(leaderStudent);
	*maxBallDistance = maxDepth(leaderStudent);
	return;
}
