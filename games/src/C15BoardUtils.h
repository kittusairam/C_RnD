...
Board * createBoard(int rows, int cols){
	...
	puzzel->rows = rows;
	puzzel->cols = cols;
	puzzel->emptyTileRow = 0;
	puzzel->emptyTileCol = 0;
	return puzzel;
}
...
void initializeBoard(Board * source, int *tobeCopiedArray, int rows, int cols){
	...
	for (int i = 0; i < rows; i++){
		for (int j = 0; j < cols; j++){
			source->table[i][j] = tobeCopiedArray[p++];
	...
	source->emptyTileRow = z_r;
	source->emptyTileCol = z_c;
}
...
int checkBoard(Board * board){
	...
	for (i = 0; i < board->rows; i++){
		for (j = 0; j < board->cols; j++){
			if (board->table[i][j] != c&&c!=16){
				return 0;
			}
			c++;
		}
	}
	if (board->table[i-1][j-1] != 0){
		return 0;
	}
	return 1;
}
