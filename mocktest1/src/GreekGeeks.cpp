...
long long countCommonStars(long long start1, long long end1,
	...
	long long res = 0;
	if (start1 > end1){
		long long temp = start1;
		start1 = end1;
		end1 = temp;
	}
	if (start2 > end2){
		long long temp = start2;
		start2 = end2;
		end2 = temp;
	}
	if (start2 <start1){
	...
	start1 += start1; end1 += start1; start2 += start1; end2 += start1; p += start1;
	if (end1 < start2){
		res = 0;
	}
	else if (end1 == start2&&end1 != p){
		res = 1;
	}
	else if (end1 > start2){
		long long var = (start2 <= p&&p <= end1) ? 1 : 0;
		res = end1 - start2 - var +1;
	}
	else if (end2 < end1){
		if (start2 <= p&&p <= end2){
			res = end2 - start2;
